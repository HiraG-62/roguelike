...
| --- | --- | --- | --- | --- |
| A | implementer | src/system/xxx.ts（新規）, src/system/xxx.test.ts | state.ts: <フィールド>, tuning.ts: XXX ブロック | なし |
| B | implementer | src/render/xxxUi.ts（新規） | renderer.ts: 呼び出し 1 行 | A の型 |
| C | pixel-artist | src/data/sprites.ts | - | なし |

共有ファイルの担当: state.ts = A、game.ts = 統合役、tuning.ts = ブロックごと
統合順: A → B → C → reviewer → qa-runner
```

分割のルール
- 1 ファイルの所有者は 1 レーン。共有ファイル（state.ts / game.ts / tuning.ts / renderer.ts / main.ts）は「追加分だけの Edit」を許可として明記
