...
describe("トリガー文法", () => {
  ...
  it("同 seed で同じ効果、生成物は常に有効な組み合わせ", () => {
  ...
  it("magnitude は itemLevel でスケールする", () => {
    const average = (level: number): number => {
      const rng = createRng(77);
      let sum = 0;
      for (let i = 0; i < MANY; i++) {
        const t = generateTrigger(rng, level);
        if (t.effect === "shockwave" || t.effect === "explode" || t.effect === "chainLightning") sum += t.magnitude;
      }
      return sum;
    };
    expect(average(HIGH_LEVEL)).toBeGreaterThan(average(LOW_LEVEL) * 2);
  });

  it("AffixRoll へのエンコード → デコードで元に戻る", () => {
    const rng = createRng(13);
    for (let i = 0; i < MANY; i++) {
      const effect = generateTrigger(rng, 1 + (i % HIGH_LEVEL));
      const roll = triggerToRoll(effect, "prefix");
      expect(roll.key.startsWith(`tr_${effect.trigger}_${effect.condition}_${effect.effect}`)).toBe(true);
  ...
  it("formatTrigger が英文を作る", () => {
    ...
    ).toBe("Every 5th melee hit while at 10+ combo: release a shockwave (30 dmg)");
    expect(
      formatTrigger({
        trigger: "onKill",
    ...
    ).toBe("On kill while below 50% HP: 30% chance to gain +25% damage for 3.5s");
  });
});
