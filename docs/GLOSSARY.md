...
| 流れ「大物」（字形 大） | 精鋭（字形 精） | 世界観語の「精鋭」に統一 |
| 距離の px 表記 | m（`core/units.ts` の `formatMeters`。10px = 1m） | 表示に開発用の単位を出さない。換算の定義は表示しない |
...
- ラベル・見出し・タブ・状態表示は名詞・体言止め。システム用語を文（「〜を燃料にする」「今のビルドは関わっていない」）にしない
- 効果説明（性質・祝福・スキル・刻印符）は常体の短文。比喩より効果そのものを書く（「傷が塞がる」→「生命が回復する」）
- システムの通知・空状態・エラーはです・ます体（「倉庫は空です」）
- 擬人化・詩的な動詞で意味をぼかさない（旧「斬り伏せる・耐えて癒える」「まだ何も語らない」）。装備の色の一言は 近接 / 射撃・機動 / 守り・回復 / 会心・連撃 / 代償
- 独自語は、字面から働きが想像できるものだけ使う。状態異常（燃焼・炎上 など）と紛れる比喩は避ける
- フレーバー（人物の台詞・名のある遺物の紹介文）は雰囲気を残してよいが、意味の取れない直訳調は直す
...
| 筋力 | str | 体の性能は持たない。係数で参照する行動（武器種の段・スキルなど）の威力・怯み値だけが伸びる | `loot/resonance.ts` ATTR_LABEL |
| 技巧 | dex | 移動速度・ダッシュの再使用時間と、係数で参照する行動が伸びる | 同上 |
| 体力 | vit | 最大生命が伸び、被る状態異常の持続が縮む（状態異常への抵抗）。係数で参照する行動も伸びる | 同上 |
...
| 派生の技名 | BRANCH_NAMES | 十字断ち / 踏み込み斬り / 巴 / 逆袈裟 / 兜割り / 横一文字 / 車輪斬り / 突き崩し / 交差斬り / 乱れ斬り / 影分かれ / 十字架 / 影裂き / 石突き回し / 連ね突き / 蹴り上げ / 深突き / 刈り取り / 死の舞 / 首刈り / 引き倒し / 百裂拳 / 昇り拳 / 崩し突き / 二段蹴り / 連ね打ち / 巻き打ち / 蛇打ち / 巻き上げ / 引き裂き / 叩き落とし / 血飛沫 / 押し斬り / 骨断ち / 旋風 / 風車 / 二段払い / 打ち据え / 魔力撃 / 杖払い / 光条 / 魔力破 / 燕返し / 抜き打ち / 霞 / 峰打ち / 断ち割り / 回転斬り / 首斬り / 二丁投げ / 盾落とし / 城壁 / 盾殴り / 突進盾 / 盾押し / 巻き取り / 鎌鼬 / 引き斬り / 鎖縛り / 地砕き / 鎚車 / 打ち上げ / 鉄槌 / 二連 / 早抜き撃ち / 蹴り撃ち / 側転撃ち / 三連射 / 至近撃ち / 蹴り離し / 二度撃ち / 銃剣連突き / 銃床殴打 / 貫き撃ち / 突き撃ち / 装填撃ち / 砲身振り / 密着砲 / 突き飛ばし / 三本投げ / 回し投げ / 掴み投げ / 返し斬り / 二連弾 / 足元撃ち / 突き払い / 連置き / 罠陣 / 蹴り起爆 / 罠投げ / 三輪 / 輪回し / 輪斬り / 戻り輪（武器種ごとに 4〜5 本。左右を混ぜた 3 入力以上。盾押しは大盾の構えを離した振り） |
| 右の段の技名 | STEP2_NAMES | 受け流し / 返し斬り / 斬り上げ / 薙ぎ払い / 振り上げ / 回り斬り / 叩き伏せ / 影踏み / 交差突き / 舞い斬り / 逆手斬り / 影止め / 突進突き / 石突き / 払い / 大突き / 鎌引き / 巻き込み / 逆手回し / 断ち / 掴み投げ / 肘打ち / 膝蹴り / 回し蹴り / 正拳 / 巻き付け / 打ち払い / 地打ち / 鞭鳴らし / 肩当て / 縦割り / 横薙ぎ / 大鉈落とし / 払い上げ / 回し打ち / 天突き / 魔弾 / 大魔弾 / 杖突き / 居合 / 返し / 逆風 / 一文字 / 投擲 / 打ち割り / 回し斬り / 大割り / 構え / 盾突き / 盾叩き / 押し潰し / 分銅 / 鎌返し / 鎖回し / 鎖締め / 大薙ぎ / 振り下ろし / 横殴り / 大地叩き / 乱れ撃ち / 銃把打ち / 回転撃ち / 狙い撃ち / 銃口払い / 銃剣突き / 銃床打ち / 銃剣払い / 零距離砲 / 筒殴り / 尻叩き / 手元返し / 投げ抜け / 蹴り / 筒払い / 筒突き / 蹴り飛ばし / 撒き散らし / 罠蹴り / 起爆 / 輪払い / 輪投げ / 二輪（右の連撃の段。名前の無い段は計算式の頁で「右 N 段目」） | `data/weapons.ts` |
...
武器種ごとに 3 本（一撃 46 / 持続 23）。設計からの改名は `docs/COMBAT_DESIGN.md` A-7。名前は派生・右の段・祝福・スキルと重ねない（`data/ultimates.test.ts`）。

| 武器種 | 奥義（種類） |
| --- | --- |
| 剣（sword） | 円月（一撃） / 瞬閃（一撃） / 剣気解放（持続） |
| 大剣（greatsword） | 断罪（一撃） / 巨人の膂力（持続） / 地割り（一撃） |
| 双剣（twinBlades） | 千刃（一撃） / 朧渡り（一撃） / 残影（持続） |
| 槍（spear） | 龍穿（一撃） / 流星突き（一撃） / 陣の構え（持続） |
| 大鎌（scythe） | 魂刈り（一撃） / 冥府の渦（一撃） / 死神の間合い（持続） |
| 拳（fists） | 発勁（一撃） / 崩拳（一撃） / 闘気開放（持続） |
| 鞭（whip） | 蛇縛り（一撃） / 百鳴り（一撃） / 雷鞭（持続） |
| 鉈（cleaver） | 血肉断ち（一撃） / 首落とし（一撃） / 修羅（持続） |
| 棍（staff） | 竜巻（一撃） / 千本突き（一撃） / 柔の呼吸（持続） |
| 杖（wand） | 魔導砲（一撃） / 魔法陣（一撃） / 詠唱（持続） |
| 刀（katana） | 一刀両断（一撃） / 燕舞（一撃） / 無想（持続） |
| 斧（axe） | 血祭り（一撃） / 大投擲（一撃） / 狂斧（持続） |
| 大盾（shield） | 城門崩し（一撃） / 反撃の狼煙（一撃） / 鉄壁（持続） |
| 鎖鎌（chainSickle） | 蜘蛛の巣（一撃） / 縛り首（一撃） / 分銅回し（持続） |
| 戦鎚（hammer） | 天墜（一撃） / 砕地（一撃） / 鉄槌の律（持続） |
| 二丁拳銃（gunner） | 死の輪舞（一撃） / 神速撃ち（一撃） / 弾幕（持続） |
