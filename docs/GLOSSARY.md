...
- ラベル・見出し・タブ・状態表示・浮き文字は名詞・体言止め（「奥義終了」「鎧破壊」。「奥義が終わった」「鎧が砕けた」にしない）。システム用語を文（「〜を燃料にする」「今のビルドは関わっていない」）にしない。浮き文字は `system/floatingText.test.ts` が検査し、台詞・予告の例外はそこの `ALLOW` に理由付きで足す
...
武器種ごとに 3 本（一撃 54 / 持続 27）。設計からの改名は `docs/COMBAT_DESIGN.md` A-7。名前は派生・右の段・祝福・スキルと重ねない（`data/ultimates.test.ts`）。
...
| 爪（claws） | 爪嵐（一撃） / 首狩り跳び（一撃） / 血の疾走（持続） |
