...
| 杖 wand | 円 | 回収高 / 左右の段がすべて魔法（左 = 炎: 火矢→二連火矢→爆炎球、右 = 氷: 氷槍→長氷槍→吹雪、派生 LLR 稲妻 / RRL 毒泡 / LRL 渦巻き / RLL 閃光 / LRR 闇手 / RLR 跳ね雷） | 精神 + 霊力 | 属性を撃ち分けて副次効果を重ねる / 近接は杖先だけ |
...
| 爪 claws | 設計書 2 章 | 最速の多段・出血 | 技巧 | 張り付いて裂く / 届かない |
| チェーンアレイ flail | 設計書 3 章 | 溜め中に回して周りを打つ（spinning）/ 怯み高 | 筋力 + 体力 | 遠心の範囲 / 振りが重い |
| チャクラム ringBlades | 設計書 4 章 | 右で輪を自分の周りに回す（orbit） | 技巧 + 筋力 | 回しながら斬る / 1 打が軽い |
| 扇子 fan | 設計書 5 章 | 振りで敵弾を消す（cutsBullets）・床の地形を広げる | 技巧 + 霊力 | 風で押して弾を払う / 威力が低い |
