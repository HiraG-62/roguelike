...
| 主火力 | 左右クリック | **スキル**（4 スロット）。左クリックは「気力を溜める手段」、右クリックは「弱い遠距離 + 少量回収」（※ 2026-09-24 の武器の作り直しで右クリックは武器ごとの固有技に確定。A-7・`docs/ideas/weapon-redesign.md`） |
...
**ボタンの役割**（2026-09-24 作り直し。`docs/ideas/weapon-redesign.md`）: 左 = `MovesetDef.primary`（`PrimaryKind`）、右 = `MovesetDef.art`（固有技 `WeaponArtDef`）。左は melee = 押すたびに連撃の次の段 / charge = 長押しで溜め、離して振る（大剣・戦鎚。tap は連撃）/ shot = 押している間、ベースの射撃の型で撃つ（**銃の家系だけ**。`isGun`）。近接の武器種は撃てない。右の固有技は 5 種: strike（1 振り。`["secondary"]` の派生として `branches` に混ざる）/ charge（長押しで溜める。刀の居合・短銃の狙い撃ち）/ hold（押している間の構え。剣の受け流し・大盾の構え）/ throw（射撃の型を借りて弾を出す。射撃扱い）/ recall（自分の弾を手元へ戻す）。技ごとの再使用は `Player.art.cooldown`、再使用中の右は何もせず入力列にも積まない。実装は `system/weaponArts.ts`（受け流し・構えは `combat.ts` の `damagePlayer` が `tryParry` / `guardDamageMul` を呼ぶ）。

**コンボ派生**（`MovesetDef.branches`）: 左 = primary、右 = secondary の押した瞬間を `attack.inputs` に積み（`WEAPON.chainWindow` 秒で途切れる。連撃の終わりで空にする）、末尾が派生の `sequence` に一致したら次の振りを派生に差し替える（長い列を先に照合。振っている最中なら今の振りの後に出す）。`next` がある派生は連撃がその段から続き、無いものはフィニッシュ。右の押した瞬間は `Player.secondaryWasHeld` との差で取る（入力の形式は変えない）。strike の技と構えを離した振り（`BranchDef.art`）は `defineMoveset` が派生に混ぜる。構えを離した振り（release）は押した瞬間には照合しない。「右→左」の派生は「技 → 左」の追い打ちとして残る。

| 武器種 | 段（多段は ×n） | 右の固有技（種類） | 派生（入力 → 技） |
| --- | --- | --- | --- |
| 剣 | 箱 3 段 | 受け流し（hold。窓 0.25 秒・怯み値 40・カウンター扱い、外すと 0.2 秒硬直、再使用 0.6 秒） | 左左右 → 十字断ち / 右左 → 踏み込み斬り（受け流し → 追い打ち、→ 2 段目） |
| 大剣 | 扇 ×3 → 円（左の長押しで溜め 3 段階） | 薙ぎ払い（strike → 3 段目） | 左左右 → 兜割り |
| 双剣 | 箱 ×2 → 箱 ×2 ヒット → 箱 → 交差（heavy）の 5 段 | 影踏み（strike。踏み込み中 0.1 秒無敵 → 2 段目） | 左左右 → 乱れ斬り（円 ×4）/ 左左左右 → 交差斬り（×2） |
| 槍 | 突き ×2 → 二連突き（×2）→ 突き（heavy） | 突進突き（strike。踏み込み 40・穂先判定・heavy → 2 段目） | 左左右 → 石突き回し（円） |
| 大鎌 | 扇 160°（引き寄せ）×2 → 扇 200°（引き寄せ ×2）→ 扇 270° | 鎌引き（strike。突き 44・引き寄せ → 2 段目） | 左左右 → 刈り取り（360°） |
| 拳 | 箱 ×3 → 連打（×3）→ 箱（heavy）の 5 段 | 掴み投げ（strike。背後へ放る・heavy） | 左左右 → 昇り拳 / 左左左左右 → 百裂拳（×6） |
| 鞭 | 突き → 扇 120° → 扇 180°（×2）→ 長い突き | 巻き付け（strike。引き寄せ・恐怖 1 秒 → 2 段目） | 左左右 → 巻き打ち（円 ×3） |
| 鉈 | 箱（heavy）×4 | 肩当て（strike → 2 段目） | 左左右 → 叩き落とし（円） |
| 棍 | 扇 180° ×2 → 突き → 円 | 払い上げ（strike。ノックバック 360 → 2 段目） | 左左右 → 旋風（円 ×3） |
| 杖 | 箱 ×3 → 円（左で打つ） | 魔弾（throw。単発の型・魔法 / 光・再使用 0.35 秒） | 左右 → 魔力撃（打ってから撃つ）/ 右右左 → 杖払い |
| 刀 | 箱 ×3 → 突き（heavy） | 居合（charge。突き 60、2 段階） | 左左右 → 燕返し（箱 ×2）/ 右左 → 抜き打ち（→ 2 段目） |
| 斧 | 扇 120° ×3 → 扇 140°（heavy・出血 2） | 投擲（throw。回転刃の型で行って戻る・再使用 1.2 秒） | 左左右 → 断ち割り / 右左 → 回転斬り（円 ×2 → 3 段目） |
| 大盾 | 箱 ×3 → 箱（heavy）。威力低・怯み値とノックバック大 | 構え（hold。前方 120° の被ダメ ×0.3・受けるたび必殺ゲージ・移動 ×0.5・最長 2 秒。離すと盾押し → 2 段目） | 左左右 → 盾落とし（円） |
| 鎖鎌 | 扇 100° ×3 → 扇 120°（heavy）。速い | 分銅（strike。突き 64・引き寄せ・崩勢 → 2 段目） | 左左右 → 巻き取り（円・引き寄せ ×2） |
| 戦鎚 | 円 ×2 → 箱（heavy）×2（左の長押しで溜め 2 段階） | 大薙ぎ（strike。扇 200° → 3 段目） | 左左右 → 地砕き（円 60） |
| 短銃 | 近接の段なし（左で撃つ） | 狙い撃ち（charge。移動 ×0.3、0.4 秒溜めて離すと威力 ×2・貫通 +1 の 1 発） | ジョブ固有の派生だけ |
