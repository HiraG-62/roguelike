...
| ~~共通最低間隔~~ | ~~`gcd`~~ | 2026-09-24 に廃止（B-2・B-9）。スキルの待ちはスロットごとの最低間隔と CD だけ |
...
**段の定義**（`MeleeStepDef`）: windup / active / recover・`Scaling`・怯み値・reach / size・ノックバック・heavy（壁叩きつけ）・命中 1 体ごとのマナ・当たり判定の形・引き寄せ（pull）・投げ（throw）。剣は `PLAYER.melee` / `ACTION.dashAttack` / `MANA.onMelee` をそのまま読む（数値の定義元は変えない）。
...
| 武器種 | ダッシュ攻撃 | 怯み / マナ / 移動 | 参照ステータス | 得意 / 不得意 |
| --- | --- | --- | --- | --- |
...
| 槍 spear | 突進突き（heavy） | 穂先（先端 1/3）で怯み ×2・威力 ×1.15・マナ ×1.5 | 技巧 + 筋力 | 間合い / 横と群れに弱い |
...
4. CD 型で `chargesLeft <= 0` → 既存どおり「冷却中」
5. マナ型で `mana < cost` → **不発**: 何も消費しない、`notReady(state, "マナ不足")`（既存の 0.6 秒抑制）、`pushSfx(state, "manaEmpty")`、HUD のマナバーの不足分を 0.3 秒点滅（`rs.manaFlash`）。先行入力は破棄
6. 発動 → マナ型はコストを払う、`slot.intervalLeft = minInterval`（**撃ったスロットだけ**。他のスロットには何も立てない）
...
- スキル枠 4 つ（16×16、間隔 4）。枠下にキー、右上にコスト数値。マナ不足の枠は暗く、CD 型は既存の CD マスク、最低間隔中は枠の縁が細く点滅し、枠の下端に残りの細いバー（撃ったスロットだけ減る）
...
- **共通最低間隔（GCD）は廃止**。各スロットは自分の `minInterval` と CD 型の CD / チャージだけで撃てる
- **同じステップに押した複数スロットは、入力順 1→4 で全部受け付ける**（`updateSkills` の `pressed.forEach`。決定性のため順序を固定）。マナは先に撃ったスロットから払うので、足りない分は後のスロットが不発になる
...
- 溜め（刻印符）も同じ規則: 溜め始め・離した瞬間に body の排他にかかれば不発（チャージ・マナは未消費）
...
- **ドロップ**: 部屋クリア（既存 30%）に加え、撃破時に `rollRuneDrop(rng, depth, source)`。確率は `SKILL.drop.runeOnKill`（通常 1%〔深度で +0.1%/層、上限 +2%〕/ エリート 12% / ボス 60% / 図書館の敵 5% / 巣窟の敵 4%）。乱数は外れでも 1 回だけ引く
...
| 戦闘中の回復の共通上限 | なし | `HEAL.sustainWindow`（1 秒）あたり最大 HP の `HEAL.sustainCapRatio`（4%）まで。命中時・撃破時・祝福の撃破回復（血の饗宴・血煙・饗宴の盃）が同じ窓を共有する | `combat.ts` `healSustained` |
| HP 自然回復 `hpRegen` | 常時 | **戦闘中でない間だけ**。戦闘中 = 封鎖中の部屋がある、または `MANA.combatRadius` 内に生きた敵（死神を含む）がいる。マナの自然回復（`mana.ts` の `tickMana`）と同じ判定 | `combat.ts` `inCombat` / `hpRegenAllowed` / `tickHpRegen` |
...
| 祝福 | 血の饗宴・血煙 3 / 血の代償 全回復 | 2 / 最大 HP の 60% | tuning `BOON` |
| 誓約 | 吸血 命中時 +3 / 背水 失った HP の 50% | 与ダメの 5% / 30% | tuning `KEYSTONE` |
| 性質・名のある遺物・共鳴 | 曲線どおり | 装備の強さの係数（`docs/LOOT_DESIGN.md`「揺らぎ」の `FLUX`）を受ける。implicit の回復量は手で 30〜50% 下げた | `loot/flux.ts` / `loot/affixes.ts` |

未統合（他レーンの所有ファイル。統合時に `healSustained` へ差し替える）: スキルの吸命（`system/skills.ts`）、怯み吸い `healOnStagger`（`system/traitHooks.ts`）、トリガー効果 `heal`（`system/triggers.ts`）、HP 自然回復の呼び出し（`system/player.ts` の `tickRegen` → `tickHpRegen`）。
