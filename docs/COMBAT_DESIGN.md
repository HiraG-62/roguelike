...
| 主火力 | 左右クリック | **スキル**（4 スロット）。左クリックは「気力を溜める手段」、右クリックは「弱い遠距離 + 少量回収」 |
...
武器スロットのベースが **武器種**（`MovesetKey`）、銃スロットのベースが **射撃の型**（`ShotKey`）を決める。`BaseItemDef.moveset` / `shot` → `computeStats` が `PlayerStats.moveset` / `shot` に畳み込む（武器なしは剣、銃なしは単発）。横並びで、上位互換を作らない（`data/weapons.test.ts` が「攻撃中の移動・リーチ・秒間の威力のどれかで剣に劣る」を検査する）。
...
**入力**: 溜めは `FrameInput.attackHeld`（攻撃キー / パッド RT・A の押しっぱなし。リプレイのビット末尾に追加）。チャージ射撃は射撃の役割のボタンを離した瞬間に撃つ。溜め中は `chargeLevel` の効果音と、プレイヤーの周りの環（段で色と半径が変わる）で段を知らせる。ダッシュ・怯み・スキルの硬直で溜めと予約した派生は消える。
...
| 剣士 | 筋 +2 体 +1 精 -1 霊 -2 | 剣 / 大剣 / 双剣 | 終撃の命中で怯み値 +14 | 見切りで 3 秒ダメージ +25% | 突進斬り | 射撃の威力 -20% | 最初から |
| 狩人 | 技 +3 精 +1 筋 -2 体 -2 | 槍 / 鞭 | 予備動作中の敵への射撃で怯み値 +10 | 精鋭への射撃で脆弱 2.5 秒（ICD 1 秒） | 撃ち抜き | 最大生命 -15% | 最初から |
| 拳闘士 | 筋 +2 体 +2 技 -1 霊 -3 | 拳 / 鉈 / 棍 | 近接 6 回ごとに衝撃波（1 段目 ×0.8） | 被弾で 3 秒ダメージ +30% | 地裂き | 射撃の威力 -30% | 最初から |
...
- ステータス・得意な武器・弱点: `system/runSetup.ts` の `applyRunStats` が `system/jobs.ts` の `applyJobStats` を通す（誓約の後、振り分けと派生の前）。偏りは生値に足すので逓減は派生でまとめて掛かる。偏りの合計は 0。得意な武器種を持つ間は近接の威力 ×1.1・攻撃速度 ×1.08（`JOB.favoredMeleeMul` / `favoredAttackSpeedMul`）。`MovesetDef` は触らない
