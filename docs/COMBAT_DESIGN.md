...
| 精神 `mnd` | 金 | バースト | 最大マナ +6×d / マナ自然回復 +0.3×d 毎秒 / 会心率 +0.4%×d |
...
```
左クリック命中 ──(+4 / +4 / +7)──┐
右クリック命中 ──(+1.5、1 射撃 3 まで)─┤
ジャスト回避 ────(+15)──────────┼──▶ マナ ──▶ スキル（消費）
自然回復（戦闘中 3.5/秒、非封鎖中 ×4）─┘
```

| 定数 `MANA` | 値 | 意味 |
| --- | --- | --- |
| `baseMax` | 100 | 精神 5 のときの最大マナ（派生は A-1） |
| `baseRegen` | 3.5 | 精神 5 のときの自然回復 / 秒 |
| `idleRegenMul` | 4 | 封鎖されていない部屋・通路での自然回復倍率（待ち時間を作らない） |
| `onMelee` | [4, 4, 7] | 近接各段の命中 1 体ごと |
| `onDashAttack` | 5 | ダッシュ攻撃 |
| `onCounterMul` | 2 | カウンターヒットなら倍 |
| `onShot` | 1.5 | 射撃弾の命中 1 体ごと |
| `shotVolleyCap` | 3 | 1 回の射撃（複数弾）で回収できる上限 |
| `onJust` | 15 | ジャスト回避 |
| `onKill` | 2 | 撃破（どの手段でも） |
| `startFull` | true | ラン開始と階層到達で満タン |

- 回収は `manaGainMul`（要追加: `PlayerStats.manaGainMul`、既定 1）を掛ける
- **スキル自身の命中ではマナが戻らない**（無限ループ防止）。例外は刻印符「連鎖」（B-5）
- 1 ヒットで回収するのは近接の「命中した敵 1 体ごと」だが、1 振りあたり上限 3 体ぶん（`MANA.meleeTargetCap = 3`）
...
| 2 | ジャスト回避（ダッシュ無敵中の被弾） | **残す** | 窓は上の 0.10 秒。報酬: スロー（0.45 → 0.35 秒）、必殺ゲージ、マナ +15、`justTimer` |
...
重い攻撃（D-3 と E-4 の表で「怯み」を持つもの）は、プレイヤーに状態異常「怯み」を付ける。効果: 移動 ×0.3、攻撃・スキル・ダッシュ不可。持続は E-4 の値 × `statusTakenMul`（体力で短くなる）。被弾後の無敵 0.5 秒の中に収まる長さにする（追撃で即死させない）。
...
// DEFAULT_STATS: attributes / attributesEff は全 5、maxMana 100、manaRegen 3.5、他の倍率は 1、statusProcs []
```
