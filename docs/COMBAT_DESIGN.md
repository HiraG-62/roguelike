...
**ボタンの役割**（`MovesetDef.primary` / `secondary`、`ActionKind`）: melee = 押すたびに連撃の次の段 / shot = 押している間、銃スロットの射撃の型で撃つ / charge = 押して溜め、離して振る（tap は連撃）。剣など既定は 左 melee・右 shot（従来の操作）。大剣は 左 charge・右 melee（撃てない代わりに右が薙ぎ払い）。杖は 左 shot・右 melee。

**コンボ派生**（`MovesetDef.branches`）: 左 = primary、右 = secondary の押した瞬間を `attack.inputs` に積み（`WEAPON.chainWindow` 秒で途切れる。連撃の終わりで空にする）、末尾が派生の `sequence` に一致したら次の振りを派生に差し替える（長い列を先に照合。振っている最中なら今の振りの後に出す）。`next` がある派生は連撃がその段から続き、無いものはフィニッシュ。右の押した瞬間は `Player.secondaryWasHeld` との差で取る（入力の形式は変えない）。

| 武器種 | 段（多段は ×n） | 派生（入力 → 技） |
| --- | --- | --- |
| 剣 | 箱 3 段 | 左左右 → 十字断ち（フィニッシュ）/ 右左 → 踏み込み斬り（→ 2 段目） |
| 大剣 | 扇 ×3 → 円（溜め 3 段階） | 右 → 薙ぎ払い（→ 3 段目）/ 左左右 → 兜割り |
| 双剣 | 箱 ×2 → 箱 ×2 ヒット → 箱 → 交差（heavy）の 5 段 | 左左右 → 乱れ斬り（円 ×4）/ 左左左右 → 交差斬り（×2）/ 右左 → 影踏み（→ 2 段目） |
| 槍 | 突き ×2 → 二連突き（×2）→ 突き（heavy） | 左左右 → 石突き回し（円）/ 右左 → 飛び込み突き（→ 2 段目） |
| 大鎌 | 扇 160°（引き寄せ）×2 → 扇 200°（引き寄せ ×2）→ 扇 270° | 左左右 → 刈り取り（360°）/ 右左 → 鎌引き（→ 2 段目） |
| 拳 | 箱 ×3 → 連打（×3）→ 箱（heavy）の 5 段 | 左左右 → 昇り拳 / 左左左左右 → 百裂拳（×6）/ 右左 → 踏み込み拳（→ 2 段目） |
| 鞭 | 突き → 扇 120° → 扇 180°（×2）→ 長い突き | 左左右 → 巻き打ち（円 ×3）/ 右左 → 鞭鳴らし（→ 2 段目） |
| 鉈 | 箱（heavy）×4 | 左左右 → 叩き落とし（円）/ 右左 → 肩当て（→ 2 段目） |
| 棍 | 扇 180° ×2 → 突き → 円 | 左左右 → 旋風（円 ×3）/ 右左 → 払い上げ（→ 2 段目） |
| 杖 | 右で 箱 ×3 → 円 | 左右（撃ってから打つ）→ 魔力撃 / 右右左 → 杖払い |
| 刀 | 箱 ×3 → 突き（heavy）。右の長押しで居合（突き 60、2 段階） | 左左右 → 燕返し（箱 ×2）/ 右左 → 抜き打ち（→ 2 段目） |
| 斧 | 扇 120° ×3 → 扇 140°（heavy・出血 2） | 右 → 回転斬り（円 ×2 → 3 段目）/ 左左右 → 断ち割り（箱 heavy） |
| 大盾 | 箱 ×3 → 箱（heavy）。威力低・怯み値とノックバック大 | 右 → 盾押し（→ 2 段目）/ 左左右 → 盾落とし（円） |
| 鎖鎌 | 扇 100° ×3 → 扇 120°（heavy）。速い | 右 → 分銅（突き 64・引き寄せ・崩勢 → 2 段目）/ 左左右 → 巻き取り（円・引き寄せ ×2） |
| 戦鎚 | 円 ×2 → 箱（heavy）×2。左の長押しで溜め（円、2 段階） | 右 → 大薙ぎ（扇 200° → 3 段目）/ 左左右 → 地砕き（円 60） |
| 二丁拳銃 | 近接の段なし（左右とも射撃） | なし（ジョブ固有の派生だけ出る） |
